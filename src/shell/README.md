...
ctxfs-shell provides a lightweight interpreter with bash-like syntax, enabling script development.
...
- [Built-in Commands](#built-in-commands)
  ...
  - [AGFS Management](#ctxfs-management)
...
ctxfs-shell is a lightweight, educational shell that demonstrates Unix pipeline concepts while integrating with the AGFS (Aggregated File System) server. All file operations go through AGFS, allowing you to work with multiple backend filesystems (local, S3, SQL, etc.) through a unified interface.
...
- **Command History**: Persistent across sessions (`~/.ctxfs_shell_history`)
...
# Pull the image
docker pull c4pt0r/ctxfs:latest

# Run the server with port mapping
docker run -d -p 8080:8080 --name ctxfs-server c4pt0r/ctxfs:latest

# With data persistence (mount /data directory)
docker run -d -p 8080:8080 -v $(pwd)/data:/data --name ctxfs-server c4pt0r/ctxfs:latest

# With custom configuration
docker run -d -p 8080:8080 -v $(pwd)/config.yaml:/config.yaml --name ctxfs-server c4pt0r/ctxfs:latest
...
ctxfs:/> echo "Hello, World!" > /local/tmp/hello.txt
ctxfs:/> cat /local/tmp/hello.txt
Hello, World!

ctxfs:/> ls /local/tmp | grep txt
hello.txt

ctxfs:/> for i in 1 2 3; do
>   echo "Count: $i"
> done
Count: 1
Count: 2
...
```bash
# Using -c flag
uv run ctxfs-shell -c "echo 'test' > /local/tmp/test.txt"

# With pipeline
uv run ctxfs-shell -c "cat /local/tmp/data.txt | sort | uniq > /local/tmp/sorted.txt"
```
...
```bash
cat > example.as << 'EOF'
#!/usr/bin/env uv run ctxfs-shell
...
#### upload [-r] local_path ctxfs_path
...
#### download [-r] ctxfs_path local_path
...
- Prompt changes to `ctxfs[chroot]:/>` to indicate chroot mode
...
- **Persistent History**: Commands saved in `~/.ctxfs_shell_history`
- **Navigation**: Use ↑/↓ arrow keys
- **Customizable**: Set `HISTFILE` variable to change location

```bash
ctxfs:/> export HISTFILE=/tmp/my_history.txt
ctxfs:/> # Commands now saved to /tmp/my_history.txt
```
...
```bash
ctxfs:/> ec<Tab>              # Completes to "echo"
ctxfs:/> cat /lo<Tab>         # Completes to "/local/"
ctxfs:/> ls /local/tmp/te<Tab>    # Completes to "/local/tmp/test.txt"
```
...
```bash
ctxfs:/> echo "This is a \
> very long \
> message"
This is a very long message

ctxfs:/> if [ -f /local/tmp/file.txt ]; then
...
When ctxfs-shell starts, it automatically checks for and executes initialization scripts from the AGFS filesystem. This allows you to set up environment variables, define functions, aliases, and perform other initialization tasks.
...
ctxfs-shell --skip-initrc

# Use a custom initrc script (skips default scripts)
ctxfs-shell --initrc /path/to/custom_initrc.as

# Custom initrc from AGFS path
ctxfs-shell --initrc /etc/my_profile

# Custom initrc from local filesystem
ctxfs-shell --initrc ~/my_initrc.as
```

### Default Initialization File Locations

When no `--initrc` or `--skip-initrc` is specified, the following files are checked in order. If a file exists, it is executed as an ctxfs-shell script:
...
./test_simple_for.ctxfsh
./test_for.ctxfsh
./test_for_with_comment.ctxfsh
...
# Test pipelines
ctxfs:/> echo "hello world" | grep hello | wc -w

# Test variables
ctxfs:/> NAME="Alice"
ctxfs:/> echo "Hello, $NAME"
...
# Test control flow
ctxfs:/> for i in 1 2 3; do echo $i; done

# Test file operations
ctxfs:/> echo "test" > /local/tmp/test.txt
ctxfs:/> cat /local/tmp/test.txt
...
uv run ctxfs-shell --ctxfs-api-url http://192.168.1.100:8080

# Via config file
# Create ~/.ctxfs_shell_config with:
...
```bash
# Single variable
ctxfs-shell --env FOO=bar

# Multiple variables
ctxfs-shell --env FOO=bar --env BAZ=qux -c 'echo $FOO $BAZ'

# Bulk injection from shell environment
ctxfs-shell --env "$(env)" -c 'echo $PATH'

# Bulk injection from custom format (KEY=VALUE per line)
ctxfs-shell --env "$(cat my_env_file)" -c 'env'
```
...
Built with:
- [pyctxfs](https://github.com/c4pt0r/pyctxfs) - Python client for AGFS
