# AGENTS.md - ctxfs-shell (Python Shell & AST Engine)

## OVERVIEW
`ctxfs-shell` is a pure-Python shell implementation featuring a full Lexer/Parser/AST pipeline. Unlike simple wrappers, it manages its own state, control flow, and job control, using the `pyctxfs` SDK for filesystem operations.
...
def test_cat_command(mock_filesystem):
    ...
    # Execute
    from ctxfs_shell.commands.cat import cmd_cat
...
def test_echo_output(mock_process):
    # Execute command
    from ctxfs_shell.commands.echo import cmd_echo
...
**Command:**
```bash
sed -i '' 's/_mode_to_rwx/mode_to_rwx/g' ctxfs_shell/builtins.py
```
...
**Tests Performed:**
```bash
uv run ctxfs-shell -c "pwd"                    # ✓
uv run ctxfs-shell -c "env | head -3"          # ✓
uv run ctxfs-shell -c "export X=y; echo \$X"   # ✓
```
...
3. **Implemented translate_ctxfs_error()** - Automatic SDK error translation
...
```python
def translate_ctxfs_error(error: Exception, path: str) -> FileSystemError:
...
try:
    ...
    raise translate_ctxfs_error(e, path) from e
```
...
- Translation function: translate_ctxfs_error()
...
**Success Factor:** Created a solid exception foundation without disrupting existing code. The 18 new exception types provide vocabulary for precise error handling, while translate_ctxfs_error() ensures consistent behavior across the filesystem layer.
...
**Command:**
```bash
uv run black ctxfs_shell/
uv run isort ctxfs_shell/
uv run ruff check ctxfs_shell/
```
...
uv run black ctxfs_shell/ tests/
uv run isort ctxfs_shell/ tests/
...
with patch('ctxfs_shell.shell.AGFSFileSystem', return_value=mock_filesystem):
    shell = Shell()
    # Test using shell
```
...
def test_expand_feature(self, mock_filesystem):
    ...
    from ctxfs_shell.shell import Shell
    from unittest.mock import patch
...
```bash
uv run pytest tests/ --cov=ctxfs_shell --cov-report=term
```

**Focused coverage check:**
```bash
uv run pytest tests/test_expression.py --cov=ctxfs_shell.expression --cov-report=term-missing
```
