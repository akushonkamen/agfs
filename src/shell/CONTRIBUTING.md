...
git clone https://github.com/yourusername/ctxfs-shell.git
cd ctxfs-shell
...
Create a new file in `ctxfs_shell/commands/`:

```python
# ctxfs_shell/commands/mycommand.py
"""
...
Use custom exceptions from `ctxfs_shell.exceptions`:
...
def test_mycommand_basic(mock_filesystem):
    ...
    from ctxfs_shell.commands.mycommand import cmd_mycommand
    from ctxfs_shell.process import Process
    from ctxfs_shell.context import CommandContext
...
# Run with coverage
uv run pytest tests/ --cov=ctxfs_shell --cov-report=html
...
# Check code quality
uv run ruff check ctxfs_shell/ tests/
```
...
- View coverage at: `https://codecov.io/gh/YOUR_USERNAME/ctxfs-shell`
...
2. **Check code quality**
   ```bash
   uv run black --check ctxfs_shell/ tests/
   uv run ruff check ctxfs_shell/ tests/
   ```
