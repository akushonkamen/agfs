...
use ctxfs_sdk::{AgfsError, FileInfo, FileSystem, MetaData};
use chrono::Utc;
use rand::Rng;
...
impl FileSystem for DevFS {
    ...
    fn write(&self, path: &str, data: &[u8], _offset: i64, _flags: ctxfs_sdk::WriteFlag) -> Result<i64, AgfsError> {
        if !Self::is_valid_device(path) {
            return Err(AgfsError::not_found(path));
        }
...
mod tests {
    ...
    fn test_devfs_null_write() {
        let fs = DevFS::new();
        let result = fs.write("/null", b"test", 0, ctxfs_sdk::WriteFlag::NONE);
