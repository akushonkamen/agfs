...
use ctxfs_sdk::{FileSystem, Streamer, WriteFlag};
use axum::{
    body::Body,
    extract::{Query, State},
...
fn stream_response(
    _path: String,
    _stream_reader: Box<dyn ctxfs_sdk::StreamReader>,
) -> Result<Response, Response> {
...
pub struct StreamReaderBody {
    reader: Option<Box<dyn ctxfs_sdk::StreamReader>>,
    _path: String,
    timeout_ms: u64,
}

impl StreamReaderBody {
    pub fn new(reader: Box<dyn ctxfs_sdk::StreamReader>, path: String) -> Self {
        Self {
            reader: Some(reader),
            _path: path,
