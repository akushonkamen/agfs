...
#[cfg(target_os = "linux")]
use ctxfs_sdk::{Client, FileInfo, WriteFlag};
use chrono::Utc;
...
impl AGFSFS {
    ...
    async fn get_file_info(&self, path: &str) -> std::result::Result<FileInfo, ctxfs_sdk::AgfsError> {
        // Try cache first
    ...
    async fn read_dir_cached(&self, path: &str) -> std::result::Result<Vec<FileInfo>, ctxfs_sdk::AgfsError> {
        // Try cache first
