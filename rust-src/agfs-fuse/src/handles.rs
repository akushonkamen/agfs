...
impl HandleManager {
    ...
    pub async fn open_remote(&self, path: &str, flags: u32) -> Result<u64, agfs_sdk::AgfsError> {
    ...
    pub async fn open_stream(
        ...
        path: &str,
        flags: u32,
    ) -> Result<u64, agfs_sdk::AgfsError> {
        self.open_remote(path, flags).await
    }

    /// Open a local handle (fallback)
    pub fn open_local(&self, path: &str, flags: u32) -> Result<u64, agfs_sdk::AgfsError> {
        let local_id = self.allocate_handle_id();
        let info = HandleInfo {
            path: path.to_string(),
            flags,
    ...
    /// Close a handle
    pub async fn close(&self, handle_id: u64) -> Result<(), agfs_sdk::AgfsError> {
        self.handles.remove(&handle_id);
        Ok(())
    }

    /// Close all open handles
    pub async fn close_all(&self) -> Result<(), agfs_sdk::AgfsError> {
        self.handles.clear();
        Ok(())
    }
    ...
    pub async fn read_remote(
        ...
        offset: i64,
        size: i64,
    ) -> Result<Vec<u8>, agfs_sdk::AgfsError> {
        let info = self
            .get(handle_id)
            .ok_or_else(|| agfs_sdk::AgfsError::invalid_argument("invalid handle"))?;
    ...
    pub async fn write_remote(
        ...
        data: &[u8],
        offset: i64,
    ) -> Result<i64, agfs_sdk::AgfsError> {
        let info = self
            .get(handle_id)
            .ok_or_else(|| agfs_sdk::AgfsError::invalid_argument("invalid handle"))?;

        // Use SDK's write_with_flags method
        self.client.write_with_flags(&info.path, data, offset, agfs_sdk::WriteFlag::empty())
            .await?;
        Ok(data.len() as i64)
    }
